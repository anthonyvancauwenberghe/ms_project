...
import simulation.tasks.SimulationDataTask;
...
public class Simulation {
    ...
    public void run(int amount) {
        if (this.executed)
            throw new RuntimeException("simulation.Simulation already executed");
        ...
        for (int i = 0; i < amount; i++) {
            Callable<IterationSimulationResult[]> task = new SimulationDataTask(
                    i,
                    this.consumerArrivalRate,
                    this.corporateArrivalRate,
                    callFactory
            );
